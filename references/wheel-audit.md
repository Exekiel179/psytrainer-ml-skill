...
审计对象为曾随仓库分发的 `PsyTrainer-0.2.0-cp314-none-any.whl`（来源与指纹见 NOTICE.md），
逐项核对包内 79 个文件所属的公开模型、特征处理、重采样、训练、评分、
领域入口及辅助模块。以下是功能迁移，不承诺复现旧版存在泄漏或错误的数值结果。
当前版本已移除该二进制包，默认 Pipeline 不下载、安装或导入它；旧 INI 和历史模型仅保留显式外部包兼容入口。
...
wheel 本体不再随仓库和新构建的 Skill 包分发；历史来源声明保留。
领域集合已在删除前从 wheel AST 核对，后续测试使用记录的公开模型名清单并执行真实训练，不再依赖 wheel 文件。
