...
from install_runtime import ROOT, resolve_base_python, resolve_wheel, wheel_python
...
def main() -> int:
    ...
    wheel = resolve_wheel(None)
    required = wheel_python(wheel)
    base = resolve_base_python(None, required)
    ...
    with tempfile.TemporaryDirectory(prefix="psytrainer-bundle-") as temp:
        ...
        subprocess.run([
            ...
            "--dest", str(staging / "wheelhouse"), "-r", str(ROOT / "requirements.txt"), str(wheel),
        ], check=True)
        for name in ("README.md", "SKILL.md", "NOTICE.md", "LICENSE", "requirements.txt"):
